...
const STEP_TITLES = {
  ...
  tripodUsedNow: 'Trépied pour cette prise de vue ?',
};

export function SituationWizard({ initialTripodAvailable = false, onComplete }) {
  const [situation, setSituation] = useState(() => ({
    ...createEmptySituation(),
    tripodUsedNow: initialTripodAvailable,
  }));
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {currentStep === 'tripodUsedNow' && (
        <StepTripodUsedNow
          value={situation.tripodUsedNow}
          onSelect={(tripodUsedNow) => selectAndAdvance({ tripodUsedNow })}
        />
      )}
      </div>
    </div>
